...
- 같은 일을 두 번 하는 비효율을 싫어합니다.
- 기획 없이 시작하는 일을 지양합니다.
- Jira, Slack 기반으로 투명하게 일하는 환경을 선호합니다.
- 탄탄한 설계 없이 개발부터 들어가는 방식을 지양합니다.
- 레거시·스파게티 코드 구조를 지양합니다.
- 기본기 없이 복잡도만 올리는 것을 경계합니다.
- 역할과 책임이 불명확한 팀 구조를 싫어합니다.
- 리더가 기초 지식이 부족한 상태에서 의사결정하는 리더십을 선호하지 않습니다.
- 리더가 팀원이 무엇을 담당하는지 그 담당 업무의 레벨이 어느 정도인지 모르는 리더십을 선호하지 않습니다.
<br><br><br>
